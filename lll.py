...
class MainWindow(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.debugger.setListener(self.myListener)
    ...
    def closeEvent(self, event):
        reply = QtGui.QMessageBox.question(self, 'Message','Quit?', QMessageBox.Yes, QMessageBox.No)
    ...
    def doExeFileOpen(self, exeFileName = None):
        ...
        if exeFileName is None:
            return
        mainFile = self.debugger.openFile(exeFileName)
    ...
    def onStateChanged(self, state):
        if state == lldb.eStateExited or state == lldb.eStateCrashed or state == lldb.eStateSuspended:
            self.doFocuseLine(None, -1)

        process = self.debugger.getCurrProcess()
    ...
    def openSrcFile(self, srcFilePath):
        ...
        myText.lineNumberArea.BPToggled.connect(self.toggleBreakPoint)

    def toggleBreakPoint(self, lineNo):
        sender = self.sender()
        fileName = sender.fileName
        sender.breakpoints = self.debugger.toggleBreakPoint(fileName, lineNo)
        sender.repaint()
            ...
            if SBProcess.EventIsProcessEvent(event):
                ...
                if state == lldb.eStateLaunching:
                    ...
                    for thread in process:
                        ...
                        elif reason == lldb.eStopReasonWatchpoint or reason == lldb.eStopReasonPlanComplete:   
                        ...
                        elif reason == lldb.eStopReasonSignal or reason == lldb.eStopReasonException:
